...
//    FastLED.addLeds<NEOPIXEL, 13>(leds[13], NUM_LEDS);
    ...
    byte flashGet;
//    EEPROM.get(0, flashGet); //check if a number of usable strips is stored in EEPROM
//    if(!(flashGet > 0 || flashGet < 15)){ //If no valid value is stored wait for incomeing value
//      waitForNumInput();
//    }

//    EEPROM.write(0, numOfUsableStrips);
    
//    Serial.write(numOfUsableStrips);
    
//    for(int i = 0; i<NUM_STRIPS; i++){
//      for(int j = 0; j<NUM_LEDS; j++){
//        leds[i][j].setRGB(255,255,0);     //Set all LEDs to red to indicate ready to recieve num of strips
//      }
//    }
...
void loop()
{
  ...
  if(Serial.available() >= numOfUsableStrips*5){
    for(int i = 0; i<numOfUsableStrips; i++){
    ...
    numOfUsableStrips = usableStripBuffer;
    usableStripBuffer = numOfUsableStrips;
  }
}
